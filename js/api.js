...
async function handleApiRequest(url) {
    ...
    try {
        if (url.pathname === '/api/search') {
            ...
            try {
                ...
                data.list.forEach(item => {
                    item.source_name = source === 'custom' ? '自定义来源' : API_SITES[source].name;
        ...
        if (url.pathname === '/api/detail') {
            ...
            const sourceCode = url.searchParams.get('source') || 'heimuer'; // 获取来源
            ...
            try {
                ...
                return JSON.stringify({
                    ...
                    videoInfo: {
                        ...
                        source_name: sourceCode === 'custom' ? '自定义来源' : API_SITES[sourceCode].name,
...
async function handleCustomApiSpecialDetail(id, customApi) {
    try {
        ...
        if (!response.ok) {
            throw new Error(`自定义API详情获取失败: ${response.status}`);
        }
    ...
    const searchPromises = availableSources.map(async (source) => {
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error(`${source}源请求超时`)), 8000)
            );
            ...
            if (!data || !Array.isArray(data.list)) {
                throw new Error(`${source}源返回的数据格式错误`);
            }
            ...
            console.warn(`${source}源请求失败:`, error);
            return [];
        }
    });
...
async function handleMultipleCustomSearch(searchQuery, customApiUrls) {
    ...
    const searchPromises = apiUrls.map(async (apiUrl, index) => {
        try {
            ...
            if (!data || !Array.isArray(data.list)) {
                throw new Error(`自定义API ${index+1} 返回的数据格式不正确`);
            }
